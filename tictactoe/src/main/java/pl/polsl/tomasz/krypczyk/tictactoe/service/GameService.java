...
public class GameService {
    ...
    public Game move(Move move) throws GameNotFoundException {
        ...
        if (game.getWinningConditions().checkWinConditions(player)) {
            ...
            gameRepository.updateFinishStatus(game.getGameId().toString(),
    ...
    private void updateMoves(int roundCount, Game game, String positionsToString){
        if(roundCount% 2 == 1){
            movesRepository.updateFirstPlayerMoves(game.getGameId().toString(),
                    positionsToString);
        }
        else{
            movesRepository.updateSecondPlayerMoves(game.getGameId().toString(),
                    positionsToString);
        }
    }
}
